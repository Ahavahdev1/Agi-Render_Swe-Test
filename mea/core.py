...
class MEA:
    ...
    async def executar_motor_aider_turbo_async(self, t_id: str, arquivos: list, instrucao: str, mapa: dict, root_path: str = ".") -> tuple[bool, str, float]:
        # SRE Safety Filter: Passa apenas arquivos que realmente existem no disco para o Aider.
        # Isso impede que o Aider tente recriar arquivos que o SRE Shield acabou de deletar.
        arquivos_existentes = [a for a in arquivos if os.path.exists(os.path.abspath(a))]
        if not arquivos_existentes:
            return True, "No existing files to modify.", 0.0

        hint = "\n".join([f"Signature {a}: {mapa.get(a, '')}" for a in arquivos_existentes])
        ...
        arquivos_relativos = [os.path.relpath(os.path.abspath(a), work_dir) for a in arquivos_existentes]

        # SRE Language Sync: Adicionamos '--chat-language', 'english' para forçar o idioma globalmente
        cmd = [
            ...
            "--edit-format", "whole",
            "--no-auto-lint",
            "--chat-language", "english",  # <-- Força o Aider a falar e escrever apenas em inglês
    ...
    def _discover_relevant_tests(self, afetados: list, root_path: str) -> list:
        """SRE Heuristic: Força cobertura total de integração para evitar pontos cegos de testes unitários."""
        test_folder = os.path.join(root_path, "tests")
        if not os.path.exists(test_folder):
            return []
        
        # Como engenharia global de SRE, para garantir que acoplamentos de CLI e assinaturas 
        # cruzadas (como main.py vs config.py) sejam testados, sempre executamos a pasta completa de testes.
        # Isso elimina pontos cegos de testes unitários isolados.
        return [test_folder]
        ...
        while True:
            ...
            try:
                ...
                for attempt in range(max_retries):
                    ...
                    # --- 🛡️ SRE AUDITOR ADVERSARIAL PATTERN GATE ---
                    self.log_evento("🛡️ SRE Auditor: Auditing execution state, file drift, and test logs...")
                    auditoria = await self.agents.auditor.audit_execution(
                        prompt, list(set(arqs_afetados)), test_output, mapa
                    )
                    
                    if auditoria.get("status") == "APPROVED" and test_success:
                        self.log_evento(f"✅ SRE Auditor: Approved! TDD Status: Green! {test_output}")
                        break
                    
                    self.log_evento(f"⚠️ SRE Auditor: Rejected! Reason: {auditoria.get('reason')}")
                    
                    # 1. Deletar arquivos fantasmas/alucinados indicados pelo Auditor
                    del_files = auditoria.get("files_to_delete", [])
                    for df in del_files:
                        df_clean = df.replace("workspace/", "").split("/", 1)[-1] if "workspace" in df else df
                        df_path = os.path.abspath(os.path.join(root_real, df_clean))
                        if os.path.exists(df_path):
                            self.log_evento(f"🛡️ SRE Shield: Deleting hallucinated file: {df_clean}")
                            try:
                                os.remove(df_path)
                            except Exception:
                                pass
                            if df_path in arqs_afetados:
                                arqs_afetados.remove(df_path)
                                
                    # 2. Executar o reparo autônomo baseado na instrução estrita do Auditor
                    repair_instr = auditoria.get("repair_instruction") or "Fix any discrepancies and tracebacks."
                    self.log_evento(f"🧠 SRE Planner: Analyzing traceback and repository structure for repair plan...")
                    
                    reparo_plano = await self.agents.architect.planejar_reparo_universal(
                        test_output + f"\nSRE Auditor Directive: {repair_instr}", mapa, contexto, list(set(arqs_afetados))
                    )
                    ...
                    if reparo_tarefas:
                        ...
                        await self.executar_motor_aider_turbo_async(
                            f"repair_{attempt}", list(set(arqs_afetados)), repair_instr, mapa, root_path=root_real
                        )
                
                if not test_success:
                    self.log_evento("❌ SRE Alert: Auto-Repair reached max retries. Manual intervention needed.")
